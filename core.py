            ...
            if username and device_id:
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        if not data.get("valid", True) and not data.get("success", True):
                            ...
                            try:
                                # Nếu root đã được khởi tạo
                                if 'root' in locals() or 'root' in globals():
                                    root.after(0, lambda: [
                                        messagebox.showerror("Bị Đăng Xuất", "Tài khoản của bạn đã được đăng nhập từ một thiết bị khác! Bot sẽ tự đóng."),
                                        os._exit(0)
                                    ])
                                else:
                                    messagebox.showerror("Bị Đăng Xuất", "Tài khoản của bạn đã được đăng nhập từ một thiết bị khác! Bot sẽ đóng.")
                                    os._exit(0)
                            except:
                                os._exit(0)
                            break
